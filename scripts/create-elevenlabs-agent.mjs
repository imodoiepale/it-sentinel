...
const TOOLS = [
  ...
  ["close_app", "/v1/voice/close",
   "Close an application that is running on a branch machine. Use for 'close Notepad on Lagos', 'shut that down', 'get rid of it'. Asks the window to close first so unsaved work still prompts, then forces it. The list of closable apps is NARROWER than launch_app's: some launched apps are shell-hosted and cannot be closed individually.",
   { branch: str("The branch name."), app: { type: "string", enum: ["notepad", "calculator", "chrome", "edge", "camera", "taskmanager"], description: "Allowlisted closable application id - a narrower set than launch_app accepts." }, hostname: str("Optional hostname or fragment, when the branch has more than one machine.") }, ["branch", "app"]],
